...
module.exports.nlp = async (api, ...args) => {
  ...
  const actionRegex = /(buy|sell)/i;
  const quantityRegex = /(\d+(\.\d+)?)\s*([a-zA-Z]+)/i;
  const ofRegex = /of\s*([a-zA-Z]+)/i;
  const atRegex = /@\s*(\d+(\.\d+)?)/i;

  const actionMatch = input.match(actionRegex);
  const quantityMatch = input.match(quantityRegex);
  const ofMatch = input.match(ofRegex);
  const atMatch = input.match(atRegex);

  if (!actionMatch || !quantityMatch) {
    console.log("Sorry, I couldn't understand your command. Please use a format like 'buy 100 euro of bitcoin'.");
    return;
  }

  const action = actionMatch[1];
  const quantity = quantityMatch[1];
  let currency = quantityMatch[3].toLowerCase();
  ...
  let counterCurrency;
  if (ofMatch) {
    counterCurrency = ofMatch[1].toLowerCase();
    if (currencyMap[counterCurrency]) {
      counterCurrency = currencyMap[counterCurrency];
    }
  }


  const price = atMatch ? atMatch[1] : null;

  let command = `cm trade ${action} ${quantity} ${currency}`;
  if (price) {
    command += ` @${price}`;
  }
  if(counterCurrency){
    command += ` ${counterCurrency}`;
  }
